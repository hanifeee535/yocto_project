...
static struct device_attribute dev_attr_evil = {
    ...
    },   // Set permissions for read and write
    .show = show_evil,
    .store = store_evil,
};
...
static int32_t __init evil_init(void)
{
    ...
    // Allocate memory for tasklet structure
    tasklet  = kmalloc(sizeof(struct tasklet_struct),GFP_KERNEL);
    if(tasklet == NULL) {
        printk(KERN_INFO "EVIL: Memory alocation failed");
    }
 ...
 error_sysfs_create:
    kobject_put(evil_kobj); // Remove the kobject if sysfs file creation fails
...
{
    // Ensure that the tasklet is killed before cleanup
    tasklet_kill(tasklet);
    kfree(tasklet);

    kobject_put(evil_kobj); // Remove the kobject
    sysfs_remove_file(evil_kobj, &dev_attr_evil.attr);// Remove the sysfs file
